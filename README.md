...
> **超大毫秒整数（无位数上限）**：`time` 按**任意精度**整数处理，不设十进制
> 位数上限。这一承诺同样覆盖 Python 3.11.4+/3.12 默认的「整数字符串转换安全
> 阈值」（`sys.set_int_max_str_digits`，默认 4300 位）：后端只在 JSON 解析、
> 报错信息格式化与响应序列化这三处临时放开阈值，解析时以 `parse_int=int`
> 直接保留整数字面量，其余流程仍维持解释器默认阈值。因此 **4300 位与 4301
> 位**（甚至上万位）的时间戳都会返回 200，且逐位不变。前端手写的 JSON 解析
> 器把整数数字字面量保留为 `BigInt`，并将两个文本框里的数组**原始文本**直接
...
> double 舍入而被误报为重复；真正相等的超大整数仍会被准确拦截。全过程时间戳
> 不会被转成浮点数，十进制值在请求、DP 代价计算与响应中保持一致。
...
锚点的固定入列、区间最优、越界/重复/交叉单次报错与旧请求兼容性，以及
任意精度时间戳：对 Python 3.12 默认整数转换阈值的边界值 4300 位、超阈值
4301 位与 10000 位分别发起**真实 API 请求**断言 200 且返回数字逐位一致、
超阈值时间戳真实经过锚点代价计算与 JSON 序列化、违规超大整数/锚点索引
返回唯一定位的结构化 4xx；并在内置真实 Chromium 中完成前端 `BigInt`
的输入、提交与结果读取往返），通过后退出码为 0：
...
非法 JSON 的行列号定位、空数组零代价交接，锚点的标记固定、时间漂移
下锚点不被推翻、取消锚点恢复原时间轴、越界锚点的逐条报错与输入保留，
以及 **4301 位时间戳的真实 HTTP 与浏览器 BigInt 往返**（200、逐位一致、
违规重复时唯一定位 `left[1].time`，均不得出现 500/`Internal Server Error`）。
...
| time 缺失/非整数（浮点数/布尔/字符串）/重复/非递增 | 422 | `left[2].time` |
| time 为任意长度整数（含 4300、4301、上万位） | 200 | — 无位数上限，逐位原样返回 |
...
│   │   ├── intlimit.py       # 任意精度整数：临时放开 PEP 682 阈值的解析/序列化
│   │   └── main.py           # FastAPI、手工 JSON 解析、单次失败契约
│   ├── tests/                # pytest：算法对拍、校验、真实 ASGI API、超大整数
...
│   └── nginx.conf
├── verify/
│   ├── verify.py             # 仅用标准库(+Playwright 浏览器)的联调验收脚本
│   └── Dockerfile            # 官方 Playwright-Python 镜像（真实 Chromium）
